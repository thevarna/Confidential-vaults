...
const TokenDropdown: React.FC<{
	...
	return (
		<DropdownMenu.Root open={dropdownOpen} onOpenChange={setDropdownOpen}>
			<DropdownMenu.Trigger className='w-full px-6 py-4 font-mono text-sm text-white/60 hover:text-white hover:bg-white/5 border-white/5 hover:border-white/50 flex items-center justify-between focus:outline-none'>
							...
							>
								{tokens
									...
									.map((token) => (
										<DropdownMenu.Item
											key={token.symbol}
											className='block w-full text-left px-4 py-6 text-sm text-white hover:bg-white/5'
											onClick={() => {

...
const Button: React.FC<ButtonProps> = ({ loading, status, selectedToken, onClick, address, amount, balance }) => {
	const isDisabled = loading || !amount || !address || balance < Number(amount) || address.length !== 42 || amount === '0';
	...
	if (amount && address && balance >= Number(amount) && address.length === 42 && amount !== '0') {
